...
- **Normal `graymatter init` preserves customized GrayMatter MCP entries.** The seven auto-wired clients now retain existing custom commands, arguments, environments, remote endpoints and disabled entries by default. `--only CLIENT --replace-mcp` deliberately replaces only selected GrayMatter nodes. Config edits preserve other nodes and require valid, editable documents; unsafe leaves and unsupported metadata fail without truncation. On Windows, any edit to an existing setup file, including instructions, requires permission to read its audit SACL; otherwise preflight rejects it. Edit the file manually or omit the blocked action (for example, `--skip-instructions`). Explicit `--dir` persists an absolute destination in newly installed MCP and hook commands; omitted `--dir` remains dynamic.
- **Init reports a versioned setup result with `--json`.** The command validates every requested action before applying any, reports per-action status and `runtime_verified:false`, and exits nonzero on incomplete setup by default. `--best-effort` permits exit 0 for a known partial apply with a successful required action; validation, total failure, output errors and uncertain effects still fail. Successful earlier files remain in place for retry. Interactive Windows setup now asks separately for PATH with No as the default, and `--no-path` prevents the update.

- **`graymatter init --store-only` prepares a project's memory directory without client setup** ([#81](https://github.com/angelnicolasc/graymatter/issues/81)). It creates `.graymatter/MEMORY.md` only when neither a regular marker nor `gray.db` is present; an existing database remains untouched. The repeatable operation does not write MCP configs, instructions or hooks, change `PATH`, or open the runtime. `--dir` selects the store, `--quiet` suppresses success text, and `--json` reports `created` or `already_prepared` with the selected path and marker. Versioned Bash and PowerShell launchers in [`examples/claude-global/`](examples/claude-global/README.md) prepare the Git checkout or worktree root before starting Claude Code. The command does not verify database health or client routing, and a user-scoped MCP server can still create `gray.db` when it starts.

- **`graymatter mcp serve --no-create` requires a prepared store before MCP startup** ([#81](https://github.com/angelnicolasc/graymatter/issues/81)). This opt-in flag admits an existing regular `gray.db`, or a regular `MEMORY.md` when the database is absent; it does not make the runtime read-only or certify database health. An unprepared stdio or HTTP server exits with an actionable error before creating a token, starting a daemon, or serving tools. Prepare with `init --store-only`, then reconnect. Existing MCP registrations are not silently changed. Before downgrading to a binary without this flag, remove it from opted-in registrations; an unknown flag fails visibly.
...
- **MCP stdio and Claude hooks now choose one project root and store per invocation.** `CLAUDE_PROJECT_DIR` takes priority, followed by the hook payload `cwd` for hooks, then the process working directory. An omitted `--dir` uses `<project root>/.graymatter`; an explicit relative `--dir` still uses the process working directory. For example, a hook launched in `/work/repo/subdir` with `CLAUDE_PROJECT_DIR=/work/repo` now uses store `/work/repo/.graymatter` and namespace `repo`, where the prior process-directory path could use `/work/repo/subdir/.graymatter` and `subdir`. A different legacy store containing data causes an explicit routing conflict; no facts are moved. Select the intended store with an absolute `--dir`, and inspect old namespace IDs before any manual migration. HTTP keeps its service-root routing, authentication policy, and explicit `agent_id` requirement. See [project routing and migration](docs/runtime-project-routing.md).

- **Hooks and MCP now reject a nonregular `gray.db` before opening a store.** A symbolic link, reparse point, directory, device, or inspection error blocks both eager and guarded startup, even beside a regular `MEMORY.md`; an absent DB remains allowed for default eager startup. Valid aliases of the data directory itself remain supported. This leaf check does not detect hardlinks to a regular DB or protect against filesystem replacement after inspection. HTTP still uses bearer authentication by default, with `--no-auth` limited to loopback.
